...
void coloringComponents(const std::vector<lli> &componentData, ColoringResult (*coloringAlgorithm)(lli, lli**), std::vector<lli> &colors, std::vector<lli> &labels, lli &chromaticNumber) {
    int currentIndex = 0, submatrixSize;
...
ColoringResult coloringHybrid(const int &processId, const lli &n, lli **graph, ColoringResult (*coloringAlgorithm)(lli, lli**)) {
    std::vector<lli> colors, labels;
    lli chromaticNumber = 0;
    int processSize, sumSubmatrixSize;
    MPI_Request request;
    MPI_Status status;
    ...
    if (processId == 0) {
        ...
        coloringComponents(receivedComponent, coloringAlgorithm, colors, labels, chromaticNumber);
    }
